...
#####################################################################################
"""
		Concat coarse information using late fusion
		Python 3
		Pytorch 1.1.0
		Author: Neelu Madan
...
class UNetLate(nn.Module):
    def __init__(self, kernel_size=3, padding=1):
        super(UNetLate, self).__init__()
        print("RUNNING Late fusion")
        self.conv1_1 = nn.Conv2d(1, 64, kernel_size=kernel_size, padding=padding) # Now the input contain 5 slices i.e. 5 for eac class segmentation and 1 is for input
        ...
        # late fusion
        self.conv_fusion = nn.Conv2d(261, 256, kernel_size=1)
        self.conv_fusion_bn = nn.BatchNorm2d(256)
    ...
    def forward(self, x, seg):
        ...
        combined_in = torch.cat((seg, conv3), dim=1)
        conv3_fused = F.relu(self.conv_fusion_bn(self.conv_fusion(combined_in)))
